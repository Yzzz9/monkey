...
pub fn eval(node: Node, env: Rc<RefCell<Environment>>) -> Rc<Object> {
    match node {
        Node::Program(prgm) => match eval_program(&prgm.statements, env) {
...
fn eval_expression(
    ...
    match expr {
        ...
        Expression::Prefix(expr) => {
            ...
            eval_prefix_expression(expr.operator.literal.clone(), &rhs)
        }
        Expression::Infix(expr) => {
            let lhs = eval_expression(&expr.lhs, env.clone())?;
            let rhs = eval_expression(&expr.rhs, env.clone())?;
            eval_infix_expression(expr.operator.literal.clone(), &lhs, &rhs)
        }
